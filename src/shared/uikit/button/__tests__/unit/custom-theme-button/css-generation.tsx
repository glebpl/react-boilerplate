...
const theme: EmotionTheme = { mode: 'light', platform: 'cloud' };

const base: CSSObject = getCss(theme, {
  appearance: 'default',
  spacing: 'default',
  isSelected: false,
...
interactions.forEach((interaction: InteractionState) => {
  ...
  it(`should roll up [${selector}] styles to the top level`, () => {
    const result = getCustomCss({ state: interaction })(theme);
...
it('should roll up the loading styles when loading', () => {
  ...
  const result = getCustomCss({ isLoading: true, state: 'default' })(theme);

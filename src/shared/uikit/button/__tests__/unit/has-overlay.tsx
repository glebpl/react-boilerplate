  ...
  describe(`has overlay [type: <${tag}>]`, () => {
    ...
    bindings.forEach((binding: Binding) => {
      it(`should block and prevent [${binding.eventName}]`, () => {
        ...
        expect(firstEventAllowed).toBe(true);
        ...
        // jsdom handles 'touchstart' wrong
        if (binding.eventName !== 'touchstart') {
...
it(`should ignore pointer-events on all children when button has overlay`, () => {
  const buttonOnClick = jest.fn();
  const { getByTestId } = render(
    <Button testId='button' isDisabled onClick={buttonOnClick}>
      Hello
    </Button>
  );
  const button: HTMLElement = getByTestId('button');
  ...
  expect(
    hasStyleRule(`.${button.className}[data-has-overlay="true"]>*`, {
      pointerEvents: 'none'
    })
  ).toBe(true);
});

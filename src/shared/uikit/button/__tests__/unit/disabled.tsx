...
types.forEach((tag: React.ElementType) => {
  describe(`disabled [type: <${tag}>]`, () => {
    ...
    it(`should ignore pointer-events on all children when disabled`, () => {
      ...
      expect(
        hasStyleRule(`.${button.className}[disabled]>*`, {
          pointerEvents: 'none'
        })
      ).toBe(true);
    });
    ...
    bindings.forEach((binding: Binding) => {
      it(`should block and prevent [${binding.eventName}]`, () => {
        ...
        // jsdom handles 'touchstart' and 'mouseup/mousedown' on button wrong
        if (
          binding.eventName !== 'touchstart' &&
          (tag !== 'button' || (binding.eventName !== 'mousedown' && binding.eventName !== 'mouseup'))
        ) {
